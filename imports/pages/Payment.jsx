...
export const Payment = (props) => {
  ...
  // Email
  const emailFrom = "Official Ruqon <official.ruqon@gmail.com>";
          ...
          if (error) console.log("error", error);
            ...
            if (inputState.targetInfo.target_email) {
              Meteor.call(
                ...
                emailFrom,
                "[PEMBERITAHUAN!] Anda Telah Ditargetkan",
                `${inputState.targetInfo.target_name}, Anda telah ditargetkan ruqyah online, bersiaplah!`,
                (err) => {
                  if (err) console.log(err);
                  else
                    setFloatingAlert({
                      show: true,
                      message: "Email pemberitahuan terkirim!",
                      type: "success",
                    });
                }
              );
            }
            navigate("/ruqyah-online/success", { replace: false });
          }
        }
      );
    } else if (props.for === "kirim-doa") {
      await Meteor.call(
        "insertActivityPelet",
        inputState.featureId,
        inputState.targetInfo,
          ...
          if (error) console.log("error", error);
            ...
            if (inputState.targetInfo.target_email) {
              Meteor.call(
                ...
                `${inputState.targetInfo.target_name} <${inputState.targetInfo.target_email}>`,
                emailFrom,
                "[PEMBERITAHUAN!] Anda Mendapat Kiriman Doa",
                `${inputState.targetInfo.target_name}, Anda mendapat kiriman doa dari seseorang!`,
                (err) => {
                  if (err) console.log(err);
                  else
                    setFloatingAlert({
            ...
            navigate("/kirim-doa/success", { replace: false });
          }
        }
      );
      ...
      >
        <div className="d-flex flex-column">
          <div className="d-flex flex-column align-items-center my-5">
            <h1 className="f-ts mb-4">Metode Amalan</h1>
